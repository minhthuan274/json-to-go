...
function jsonToGo(json, typename)
{
  ...
  json = json.replace(/ObjectId\("[a-zA-Z0-9]+"\)/g, "\"ObjectId\"")
  json = json.replace(/ISODate\("\d{4}-\d\d-\d\dT\d\d:\d\d:\d\d(\.\d+)?(\+\d\d:\d\d|Z)"\)/g, "\"ISODate\"")
  console.log(json)
  try
  {
    ...
    switch (typeof val)
    {
      case "string":
        if (/\d{4}-\d\d-\d\dT\d\d:\d\d:\d\d(\.\d+)?(\+\d\d:\d\d|Z)/.test(val))
          return "time.Time";
        else if (/ObjectId/.test(val))
          return "bson.ObjectId"
        else if (/ISODate/.test(val))
          return "time.Time"
        else
          return "string";
      case "number":
